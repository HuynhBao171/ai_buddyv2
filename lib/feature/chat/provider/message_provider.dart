...
class MessageListNotifier extends StateNotifier<ChatBot> {
  ...
  Future<void> handleSendPressed({
    ...
    final ChatMessage message = ChatMessage(
      ...
      chatBotId: state.id,
      typeOfMessageUser: imageFilePath != null && audioId != null
          ? TypeOfMessageUser.imageAndAudio
          : imageFilePath != null
              ? TypeOfMessageUser.image
              : TypeOfMessageUser.text,
    );
    ...
    if (audioId != null) {
      ...
      await getGeminiResponseWithText(
          prompt: text!, imageFilePath: imageFilePath);
    }
  }

  Future<void> getGeminiResponseWithText({
    ...
    if (imageFilePath != null &&
        (state.typeOfBot == TypeOfBot.image ||
            state.typeOfBot == TypeOfBot.audio)) {
  ...
  Future<void> getGeminiResponseWithAudioAndImage({
    required String prompt,
    String? imageFilePath,
  }) async {
    final List<Parts> chatParts = state.messagesList.map((msg) {
